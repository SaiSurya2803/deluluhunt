...
function LiveMonitorNode({ team }: { team: Team }) {
  ...
  return (
    <Card className="overflow-hidden border-slate-200 shadow-sm bg-white hover:shadow-md transition-shadow">
      <div className="flex bg-slate-900 relative">
        <div className="w-1/3 aspect-video bg-black relative border-r border-slate-800 flex items-center justify-center">
          {camFrame ? <img src={camFrame} alt="cam" className="w-full h-full object-cover transform scale-x-[-1]" /> : <Video className="w-6 h-6 text-slate-700" />}
        ...
        <div className="w-2/3 aspect-video bg-black relative flex items-center justify-center">
          {screenFrame ? <img src={screenFrame} alt="screen" className="w-full h-full object-cover" /> : <Monitor className="w-8 h-8 text-slate-700" />}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-xl font-semibold text-slate-900 tracking-tight">Event Log</h2>
          {events.length === 0 ? (
            ...
            <div className="space-y-3">
              {events.map(event => {
                ...
                return (
                  <Card key={event.id} className={cn("transition-colors border-slate-200 shadow-sm", event.status === 'UNREVIEWED' ? 'bg-danger/5 border-l-4 border-l-danger' : 'bg-white opacity-70')}>
                    <CardContent className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      <div className="flex items-start gap-4">
                        <div className={`p-2 rounded-full ${event.status === 'UNREVIEWED' ? 'bg-danger/10 text-danger' : 'bg-slate-100 text-slate-400'}`}>
                          <AlertTriangle className="w-5 h-5" />
                        </div>
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-bold text-slate-900">{team?.name || 'Unknown Team'}</span>
                            <span className="text-xs text-slate-500 px-2 border border-slate-200 rounded-md bg-slate-50">{member?.name || 'Unknown'}</span>
                            {getSeverityBadge(event.severity)}
                          </div>
                          <p className="text-slate-700 font-medium">{event.event}</p>
                          <p className="text-xs text-slate-400 mt-1">{new Date(event.timestamp).toLocaleString()}</p>
                        </div>
                      </div>
                      
                      {event.status === 'UNREVIEWED' ? (
                        <Button variant="outline" size="sm" onClick={() => markReviewed(event.id)} className="bg-white border-slate-200 text-slate-600 hover:text-slate-900">
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-slate-900 tracking-tight">Active Monitors</h2>
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            {Object.values(teams).filter(t => t.quizStatus === 'ACTIVE').length === 0 && (
              <div className="col-span-full text-center p-8 text-slate-400 border border-slate-200 bg-slate-50 border-dashed rounded-xl">
